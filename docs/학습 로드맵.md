...
- [ ] volatile 키워드 역할
- [ ] 멀티스레드 서버에서 Race condition이 실제로 생기는 이유
- [ ] 컨텍스트 스위칭 비용이 성능에 미치는 영향
- [ ] 프로세스와 스레드 선택 기준
...
### 3-2. 실무 연결
- [ ] CPU와 메모리 속도 차이가 프로그램 성능에 미치는 영향
- [ ] 캐시 미스가 왜 성능에 큰 영향을 주는지
- [ ] 배열 순회와 지역성 (Locality)
- [ ] 캐시 친화적 코드가 왜 빠른지
- [ ] 비트 연산이 실제 코드에서 쓰이는 경우
...
- [ ] DB Connection 관련 실무 이슈 (고갈, 누수, 타임아웃)
- [ ] 인덱스가 있어도 쿼리가 느릴 수 있는 이유
- [ ] JOIN 전략이 성능에 미치는 영향
- [ ] 트랜잭션 격리 수준을 실무에서 어떻게 고르는지
- [ ] 커서 페이징을 언제 쓰는지
